...
export const api = {
  ...
  login: (email: string, senha: string) =>
    chamar<void>(
      async () => {
        await http.post("/sessao/login", { email, senha });
      },
      () => demo.login(email),
    ),
  logout: () =>
  ...
  criarConvite: (idosoId: number, email: string) =>
    chamar<Convite>(
      async () => (await http.post(`/idosos/${idosoId}/convites`, { email })).data,
      () => demo.criarConvite(idosoId, email),
    ),
  listarConvites: () =>
    chamar<Convite[]>(
      async () => (await http.get("/convites")).data,
      () => demo.listarConvites(),
    ),
  aceitarConvite: (id: number) =>
    chamar<void>(
      async () => {
        await http.post(`/convites/${id}/aceitar`);
      },
      () => demo.aceitarConvite(id),
    ),
